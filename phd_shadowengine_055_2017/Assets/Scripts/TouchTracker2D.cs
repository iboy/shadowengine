...
public class TouchTracker2D : MonoBehaviour {
	...
	public float  	drag 		= 	200.0f;
	public float  	angularDrag = 	200.0f;
	...
	void TouchToController ( Touch touch ) {
		...
		//Ray2D ray = Camera.main.ScreenToWorldPoint(touch.position);
...
//			string rigidbodyName = "Rigidbody dragger for Touch "+humanTouchCount.ToString();
			...
			//hit = new RaycastHit();
			if(Physics2D.Raycast(Camera.main.ScreenToWorldPoint(touch.position), Vector2.zero)) {
			//if (Physics2D.Raycast(ray, out hit)) {
				...
				if (hit.rigidbody) { // is it a rigidbody
					...
					if(!hit.rigidbody.isKinematic) {  // is it NOT set to isKinematic? i.e not to move
						...
						springJoint = TouchPointControllers[touch.fingerId].GetComponent<SpringJoint2D>();
